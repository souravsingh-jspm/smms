...
export default function TinyEditor() {
    ...
    useEffect(() => {
        fetch(`http://localhost:8000/api/conference-track/${COMMITTEE_ID_TO_EDIT}`)
            .then((res) => {
                if (!res.ok) {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/conference-track/${COMMITTEE_ID_TO_EDIT}`, {
